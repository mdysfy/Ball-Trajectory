function football()
    ...
    % Determine the maximum length of the trajectory arrays
    maxLength = min(length(t_nodrag), length(t_drag));

    % Ensure the loop runs only for the available number of points
    for k = 1:maxLength
        if k <= length(t_nodrag)
